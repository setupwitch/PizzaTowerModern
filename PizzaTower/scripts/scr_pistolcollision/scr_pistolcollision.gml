function scr_pistolcollision(_dmg, _x2 = 0, _do_linecollision = true)
{
	for (var i = 0; i < array_length(collision_list); i++)
	{
		var b = collision_list[i];
		var _inst = noone;
		if (_do_linecollision)
		{
			_inst = collision_line(x, y, _x2, y, b, false, true);
		}
		else
		{
			_inst = instance_place(x, y, b);
		}
		if (_do_linecollision && _inst == noone)
		{
			_inst = instance_place(x, y, b);
		}
		scr_pistolhit(_inst, _dmg);
	}
}

function scr_pistolhit(_inst, _dmg)
{
	var _result = false;
	with (_inst)
	{
