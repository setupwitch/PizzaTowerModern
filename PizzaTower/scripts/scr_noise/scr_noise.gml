...
function scr_noise_mach2()
{
	...
	if ((sprite_index == spr_playerN_secondjump1 || sprite_index == spr_playerN_secondjump2) && place_meeting(x + hsp, y, obj_solid))
	{
		...
		vsp = -11;
		attackspeed = 6;
		create_particle(x, y, particletypes.jumpdust);
	}
...
function scr_noise_dropstart()
{
	...
	{
		create_particle(x + irandom_range(-12, 12), y + 43, particletypes.cloudeffect);
		cloudbuffer = 5;
	}
	if (cloudbuffer > 0)
	{
...
function scr_noise_finale()
{
	with (obj_player1)
	{
		...
		if (sprite_index == spr_player_stunneddoise && ANIMATION_END)
		{
	...
	switch (substate)
	{
		...
		case states.shotgun:
			...
			if ((ANIMATION_END || _end) && minigunbuffer <= 0)
			{
				if (!doise)
				{
	...
	with (obj_player1)
	{
		...
		else if ((sprite_index == spr_playerN_squished || sprite_index == spr_bombpepend) && ANIMATION_END)
		{
			sprite_index = spr_idle;
		}
	}
