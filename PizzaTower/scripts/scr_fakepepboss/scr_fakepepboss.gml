...
function fakepep_set_attack(_phase, _current_hit, _attack, _cooldown, _deformed_timer)
{
	if (_cooldown != 0 && _deformed_timer == 0)
	{
		_deformed_timer = _cooldown;
	}
	attack_list[_phase][_current_hit] = 
	{
		attack: _attack,
		cooldown: _cooldown,
...
function fakepep_start_projectiles(_phase, _current_hit)
{
	projectile_list[_phase][_current_hit] = array_create(0);
}

function fakepep_add_projectile(_phase, _current_hit, _projectile_data)
{
	array_push(projectile_list[_phase][_current_hit], _projectile_data);
}

function fakepep_get_projectile(_phase, _current_hit, _index)
{
	return projectile_list[_phase][_current_hit][_index];
}

function fakepep_add_grabclone(_phase, _current_hit, _dir, _cooldown)
{
	fakepep_add_projectile(_phase, _current_hit, 
	{
		attack: fakepep_attacks.grab,
		cooldown: _cooldown,
		direction: _dir
	});
}

function fakepep_add_bodyslamclone(_phase, _current_hit, _dir, _cooldown)
{
	fakepep_add_projectile(_phase, _current_hit, 
	{
		attack: fakepep_attacks.bodyslam,
		cooldown: _cooldown,
		direction: _dir
	});
}

function fakepep_add_machclone(_phase, _current_hit, _dir, _cooldown, _bodyslam)
{
	fakepep_add_projectile(_phase, _current_hit,
	{
		attack: fakepep_attacks.mach,
		cooldown: _cooldown,
		direction: _dir,
		bodyslam: _bodyslam
	});
...
function fakepep_add_superjumpclones(_phase, _current_hit, _dir, _timer, _size, _emptyspot, _cooldown, _x1 = 100, _x2 = 850, _y = 402)
{
	if (_dir == -1)
	{
	...
	fakepep_add_projectile(_phase, _current_hit,
	{
		attack: fakepep_attacks.superjump,
		cooldown: _cooldown,
		direction: _dir,
		timer: _timer,
		size: _size,
...
function fakepep_add_flailingclone(_phase, _current_hit, _dir, _cooldown, _x1 = 100, _x2 = 850, _y = 402)
{
	fakepep_add_projectile(_phase, _current_hit,
	{
		attack: fakepep_attacks.throwhead,
		cooldown: _cooldown,
		direction: _dir,
		x1: _x1,
		x2: _x2,
...
function fakepep_add_tauntclones(_phase, _current_hit, _vsp1, _vsp2, _cooldown, _x1 = 125, _x2 = 825, _y = 402)
{
	fakepep_add_projectile(_phase, _current_hit,
	{
		attack: fakepep_attacks.taunt,
		cooldown: _cooldown,
		vsp1: _vsp1,
		vsp2: _vsp2,
		x1: _x1,
...
function scr_fakepepboss_do_projectiles(_phase, _hits)
{
	if (pizzahead)
	{
		exit;
	...
	var t = targetplayer;
	if (currentprojectile >= array_length(projectile_list[_phase][_hits]))
	{
		currentprojectile = 0;
	}
	var currp = currentprojectile;
	var _attack = fakepep_get_projectile(_phase, _hits, currentprojectile);
	currentprojectile += 1;
	deformed_cooldown = _attack.cooldown;
	switch (_attack.attack)
	{
		case fakepep_attacks.grab:
		case fakepep_attacks.bodyslam:
		...
		case fakepep_attacks.mach:
			...
			break;
		case fakepep_attacks.superjump:
			...
			break;
		case fakepep_attacks.throwhead:
			var xx = _attack.x1;
			if (_attack.direction == -1)
			{
				xx = _attack.x2;
		...
		else if (grounded && vsp > 0)
		{
			...
			switch (attack.attack)
			{
				...
				case fakepep_attacks.taunt:
					...
					create_particle(x, y, particletypes.highjumpcloud2);
...
function scr_fakepepboss_Sjump()
{
	...
	{
		create_particle(x + irandom_range(-25, 25), y + irandom_range(-10, 35), particletypes.cloudeffect, 0);
		steppybuffer = 8;
	}
	if (sjumpbuffer > 0)
	{
