...
function boss_update_pizzaheadKO(_hpsprite, _pal)
{
...
function scr_boss_genericintro(_introspr, _introbuffer = 30)
{
	if (sprite_index != _introspr)
	{
		sprite_index = _introspr;
		introbuffer = _introbuffer;
	}
	if (introbuffer > _introbuffer)
	{
		introbuffer = _introbuffer;
	}
	with (obj_player1)
...
function scr_boss_do_hurt_phase2(_player, _invulnerability = 100)
{
	with (_player)
	{
		state = states.phase1hurt;
		invtime = _invulnerability + 40;
		hurted = false;
		image_alpha = 1;
		alarm[5] = -1;
		alarm[6] = -1;
	...
	camzoom = 1;
	flashbuffer = _invulnerability - 40;
...
function scr_boss_playerN_phase1hurt(_func = noone)
{
