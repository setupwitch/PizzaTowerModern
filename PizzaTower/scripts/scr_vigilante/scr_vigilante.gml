...
function vigilante_start_attack(_phase, _hits)
{
	attack_list[_phase][_hits] = array_create(0);
}

function vigilante_add_attack(_phase, _hits, _attack, _cooldown = 0)
{
	array_push(attack_list[_phase][_hits], [_attack, _cooldown]);
}

function vigilante_end_attack(_phase, _hits)
{
	var found = false;
	for (var i = 0; i < array_length(attack_list[_phase][_hits]); i++)
	{
		var b = attack_list[_phase][_hits][i];
		if (b[0] == vigilante_attacks.reload || b[0] == vigilante_attacks.cowcrate)
		{
			found = true;
			break;
		}
...
function scr_vigilante_arenaintro()
{
	...
	if (!introwait)
	{
		...
		{
			introbuffer--;
			if (ANIMATION_END && (sprite_index == spr_vigilante_intro2 || sprite_index == spr_vigilante_intro2N))
			{
				image_index = image_number - 1;
			}
	...
	if (cooldown <= 0 && flickertime <= 0 && !reposition)
	{
		...
		switch (attack[0])
		{
			...
			case vigilante_attacks.dynamite:
				...
				break;
			case vigilante_attacks.cow:
				scr_vigilante_do_cow(1, attack[1]);
				break;
			case vigilante_attacks.estampede:
			...
			case vigilante_attacks.wait:
				...
				break;
			case vigilante_attacks.flamethrower:
			...
			case vigilante_attacks.uzi:
				...
				break;
			case vigilante_attacks.bazooka:
				create_particle(x, y, particletypes.highjumpcloud2);
				...
				image_index = 0;
				break;
			case vigilante_attacks.cowcrate:
				state = states.crate;
				hsp = 0;
...
function scr_vigilante_do_revolver(_shot, _revolverbuffer, _do_jump = false)
{
	shot = _shot;
	jump = _do_jump;
...
function scr_vigilante_throw_dynamite()
{
	if (shot > 0)
	{
		...
		{
			calculate_jump_velocity(obj_player.x, other.y, 18, grav);
			if (abs(hsp) <= 1 && (abs(vsp) <= 1 || vsp > 0))
			{
				calculate_jump_velocity(obj_player.x, other.y, 25, grav);
			}
		}
		return true;
	}
...
function scr_vigilante_duel()
{
	...
	var _cheat = !obj_player.ispeppino || global.swapmode;
	if (duelphase == 2)
	{
		with (obj_player)
		{
	...
	if (duelbuffer > 0)
	{
		...
		switch (duelphase)
		{
			...
			case 1:
				...
				if (obj_player.ispeppino && signy <= -sprite_get_height(signspr))
				{
					duelbuffer = 80;
		...
		if (duelphase == 2 && obj_player.ispeppino && !_cheat)
		{
...
function scr_vigilante_crate()
{
	...
	{
		shot = true;
		with (instance_create(obj_player.x, -64, obj_vigilantecrate))
		{
			cow = true;
		}
	}
