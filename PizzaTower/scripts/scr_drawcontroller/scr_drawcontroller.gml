function get_dark(_blendcolor, _dark, _do_position = false, _x = 0, _y = 0)
{
	if (_dark)
	{
		if (room == boss_vigilante)
		{
			_blendcolor = make_color_rgb(247, 109, 22);
		}
		...
		with (obj_lightsource)
		{
			if (object_index != obj_lightsource_attach || instance_exists(objectID))
			{
				...
				{
					dis = distance_between_points(x, y, _x, _y);
				}
				if (dis < d)
				{
					bb = dis / distance;
		...
		t = clamp(t, 0, 255);
		a = clamp(a, 0, 255);
		var r = (color_get_red(_blendcolor) - t) + a;
		var g = (color_get_green(_blendcolor) - t) + a;
		b = (color_get_blue(_blendcolor) - t) + a;
		if (r < 0)
		{
			r = 0;
		}
...
function enemy_is_superslam(_enemy)
{
	with (_enemy)
	{
		if (state == states.grabbed)
		{
			var g = obj_player1.id;
...
function enemy_is_swingding(_enemy)
{
	with (_enemy)
	{
		if (state == states.grabbed)
...
function draw_enemy(_draw_healthbar, _pal, _color = c_white)
{
	...
	if (visible && object_index != obj_pizzaball && object_index != obj_fakesanta && bbox_in_camera(view_camera[0], 32))
	{
		...
		if (object_index == obj_peppinoclone || (usepalette && _pal))
		{
			pattern_reset();
			shader_reset();
		}
