...
function noise_do_attack_normal()
{
	switch (state)
	{
		...
		case states.jetpackspin:
			...
			image_index = 0;
			particle_set_scale(particletypes.jumpdust, image_xscale, 1);
...
function noise_behaviour_far()
{
	...
	{
		var i = 0;
		while (current_behaviour == noise_behaviour.far)
		{
			attack_cooldown = 0;
...
function boss_noise_normal()
{
	...
	{
		case noise_behaviour.none:
			noise_behaviour_none();
			break;
		case noise_behaviour.close:
			noise_behaviour_close();
			break;
		case noise_behaviour.anywhere:
			noise_behaviour_anywhere();
			break;
		case noise_behaviour.far:
			noise_behaviour_far();
			break;
	}
}
