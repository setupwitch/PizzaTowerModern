...
function destroy_sounds(_sound_array)
{
	for (var i = 0; i < array_length(_sound_array); i++)
	{
		var b = _sound_array[i];
...
function scr_pepperman_walk()
{
	...
	{
		var shoulder = elitehit == 3 || elitehit == 4;
		var stomp = elitehit < 3;
	}
...
function pepperman_nearestspot()
{
	...
	{
		targetspot = instance_nearest(obj_player.x, obj_player.y, obj_pepper_groundpoundspot);
	}
	state = states.jump;
