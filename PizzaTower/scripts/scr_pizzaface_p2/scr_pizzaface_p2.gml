...
function scr_pizzaface_p2_normal()
{
	...
	else
	{
		if (!obj_player1.ispeppino)
		{
			with (obj_music)
			{
				if (music != noone && fmod_event_instance_get_parameter(music.event, "state") < 1)
				{
					fmod_event_instance_set_parameter(music.event, "state", 1.4, true);
				}
			}
...
function scr_pizzaface_p2_fishing()
{
	...
	else if (ANIMATION_END)
	{
		...
		else
		{
			...
			switch (t)
			{
				case pizzafacep2_attacks.pullinglevel:
					state = states.pullinglevel;
					substate = 0;
				...
				case pizzafacep2_attacks.ratball:
					...
					image_index = 0;
					break;
				case pizzafacep2_attacks.tv:
					state = states.portraitthrow;
					shot = false;
					sprite_index = spr_pizzahead_grabTV;
					image_index = 0;
					break;
				case pizzafacep2_attacks.minigun:
					...
					attackbuffer = 1;
					break;
				case pizzafacep2_attacks.pinup:
