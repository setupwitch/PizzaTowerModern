...
function scr_do_rank(_showtoppins = true, _isboss = false)
{
	...
	if (!global.tutorial_room)
	{
		...
		notification_push(notifications.level_finished, [global.leveltosave, global.secretfound, global.level_minutes, global.level_seconds]);
	...
	if (!instance_exists(obj_endlevelfade))
	{
		...
		{
			do_rank = true;
			toppinvisible = _showtoppins;
			with (obj_pizzaface)
			{
				if (bbox_in_camera(view_camera[0]))
				{
					notification_push(notifications.level_finished_pizzaface, []);
				}
			}
