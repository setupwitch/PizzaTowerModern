if (!global.pizzadelivery && global.pizzasdelivered < 5 && global.panic == false)
{
	if (!global.failcutscene)
	{
		if (global.pizzasdelivered <= 0)
		{
			...
			[
				...
				[cutscene_wait, 10],
				[cutscene_move_player, obj_player, obj_gustavo.x - 48, 6, 16],
				[cutscene_set_sprite, obj_player, 3213, 0.35, 1],
	...
	else
	{
		...
		[
			...
			[cutscene_set_sprite, obj_player, spr_player_gnomecutscene2, 0.35, 1],
			[cutscene_waitfor_sprite, obj_gustavo],
			[cutscene_set_sprite, obj_gustavo, spr_gustavo_grab, 0.35, -1],
			[cutscene_set_hsp, obj_gustavo, -12],
			[cutscene_set_sprite, obj_player, spr_player_gnomecutscene3, 0.35, 1],
			[cutscene_waitfor_sprite, obj_player],
			[cutscene_set_sprite, obj_player, spr_player_gnomecutscene4, 0.35, 1],
			[cutscene_check_collision, obj_gustavo, obj_player],
